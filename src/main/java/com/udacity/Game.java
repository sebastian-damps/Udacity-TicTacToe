...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        boolean win = false;
        char[] letter = {'x','o'};
        String[] score = {"X wins", "O wins"};

        for(int j=0;j<2;j++){ // a loop created in order to check "x" and "o" at the same time
        // Checking horizontals and verticals for wins
            for(int i = 0; i < 3; i++){
                if(grid[i][0] == letter[j] && grid[i][1] == letter[j] && grid[i][2] == letter[j] || grid[0][i] == letter[j] && grid[1][i] == letter[j] && grid[2][i] == letter[j]){
                result = score[j];
                win = true;
                }
            }
        // Checking  diagonals for wins
            if(grid[0][0] == letter[j] && grid[1][1] == letter[j] && grid[2][2] == letter[j] || grid[2][0] == letter[j] && grid[1][1] == letter[j] && grid[0][2] == letter[j]){
            result = score[j];
            win = true;
            }
        ...
        // Checking if there are no free spots and no win
        if(freeSpots == 0 && !win){
            result = "Tie";
        }
